...
export class SealClient {
	...
	// Debug method to get cached keys
	getCachedKeys() {
		return {
			entries: Array.from(this.#cachedKeys.entries()),
			size: this.#cachedKeys.size
		};
	}
	...
	async fetchKeys({
		...
		console.log('fetchKeys - Starting with:', {
			ids,
			txBytesLength: txBytes.length,
			sessionKeyAddress: sessionKey.getAddress(),
			threshold,
			cachedKeysSize: this.#cachedKeys.size
		});
		...
		for (const server of keyServers) {
			const hasAllKeys = fullIds.every((fullId) => this.#cachedKeys.has(`${fullId}:${server.objectId}`));
			console.log('fetchKeys - Checking server cache:', {
				serverId: server.objectId,
				hasAllKeys,
				cachedKeys: Array.from(this.#cachedKeys.entries())
					.filter(([key]) => key.endsWith(`:${server.objectId}`))
					.map(([key]) => key)
			});
		...
		const keyFetches = [...remainingKeyServers].map(async (server) => {
			try {
				...
				console.log('fetchKeys - Received keys from server:', {
					...
					receivedIds: allKeys.map(k => ({
						fullId: k.fullId,
						fullIdHex: Buffer.from(k.fullId).toString('hex')
					}))
				});
				...
				console.log('fetchKeys - Server key status:', {
					serverId: server.objectId,
					hasAllKeys,
					receivedIds: Array.from(receivedIds).map(id => ({
						id,
						idHex: Buffer.from(id).toString('hex')
					})),
					expectedIds: fullIds.map(id => ({
						id,
						idHex: Buffer.from(id).toString('hex')
					}))
				});
