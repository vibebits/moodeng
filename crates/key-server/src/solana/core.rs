...
fn check_instruction(
    ...
    // check ix program and cert program are the same
    if let Some(ix_program) = account_keys.get(instruction.program_id_index as usize) {
        if ix_program != cert_program {
    ...
    // check ix's first account is the cert user
    if let Some(ix_user) = account_keys.get(*instruction.accounts.get(0).unwrap() as usize) {
        if ix_user != cert_user {
            return false; // Not an instruction for our target program
        }
    } else {
        // Should not happen with a valid transaction and compiled instruction
        warn!("account_keys out of bounds in instruction");
        return false;
    }
...
pub fn get_key_ids_from_transaction(
    ...
    for ix in &transaction.message.instructions {
        // check_instruction was called earlier before simulation
        // it is called again here (TODO: do check_instruction only once?)
        if check_instruction(ix, account_keys, cert_program, cert_user) {
            ...
            let full_id = create_full_id(&cert_program.to_bytes(), id_bytes);
            println!(
...
pub async fn check_seal_approve(
    ...
    debug!(
        "check_seal_approve - cert_program: {:?}, cert_user: {:?}",
        cert_program, cert_user
    );
    let account_keys = &transaction.message.account_keys;

    // for each instruction, check it for program and user account (vs as signed in the certificate)
    let seal_instruction = transaction
        .message
        .instructions
        .iter()
        .find(|ix| check_instruction(ix, account_keys, cert_program, cert_user));
    ...
    let seal_approve_outcomes = process_simulation_result(&response_json, &cert_program.to_string())?;

    println!("### simulation seal_approve_outcomes: {:?}", seal_approve_outcomes);

    // Extract key IDs from transaction's instructions
    let key_ids = get_key_ids_from_transaction(transaction, cert_program, cert_user)?;
